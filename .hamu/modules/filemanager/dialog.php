...
echo str_replace(
    array(
        '&filter=' . ($filter ?? ''),
        '&sort_by=' . ($sort_by ?? ''),
        '&descending=' . intval($descending ?? 0)
    ),
    array('','',''),
...
function filenameSort($x, $y)
{
    ...
    if (!isset($x['file_lcase'], $y['file_lcase'])) {
        return 0;
    }
...
function extensionSort($x, $y)
{
    ...
    if (!isset($x['extension'], $y['extension'])) {
        return 0;
    }
    ...
    if ($x['is_dir'] !== $y['is_dir']) {
        ...
        return ($descending)
            ? strcasecmp($y['extension'], $x['extension'])
            : strcasecmp($x['extension'], $y['extension']);
    }
}
...
$result = str_replace('.', '', dirname($rfm_subfolder . $subdir));
echo $result ?? '';
?>"/>
